...
def main():
    ...
    dnsQuery('google.com', root_servers, 0, 'google.com')
...
def dnsQuery(name: str, root_servers: list, jumps: int, initialName: str):
    ...
    for i in range(len(root_servers)):                             # using the given name servers, try to obtain a response
        ...
        if response.answer != []:                                  # get the IP address
            ...
            if str(response.authority[0]).find(' IN CNAME ') != -1: # get the CNAME, if in recursive call, return it, if not, set name to CNAME
                name = str(response.authority[0][0])[:-1]
                initialName = name
                if jumps != 0:
                    return [name, initialName]
            elif str(response.authority[0]).find(' IN NS ') != -1: # get the NS, if in recursive call, return it, if not, set name to NS
                name = str(response.authority[0][0])[:-1]
                if jumps != 0:
                    return [name, initialName]
